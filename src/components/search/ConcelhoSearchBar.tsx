    ...
    return (
      <div className="flex items-center gap-2 rounded-lg bg-primary-50 px-3 py-2">
        ...
        <button
          ...
          className="ml-1 rounded-full p-0.5 hover:bg-primary-100 transition"
    ...
    <div className="relative">
      <div className="relative">
        ...
        <input
          ...
          className="block w-full rounded-lg border border-input bg-card py-2 pl-9 pr-9 text-sm placeholder:text-muted-foreground/70 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
          autoComplete="off"
        />
        {isLoading && (
      ...
      {isOpen && results.length > 0 && (
        <ul className="absolute z-50 mt-1 max-h-48 w-full overflow-auto rounded-lg bg-card py-1 shadow-lg ring-1 ring-ink/5">
          {results.map((concelho) => (
            ...
            >
              ...
              <span className="text-muted-foreground ml-1">({concelho.distrito})</span>
            </li>
          ))}
        </ul>
      )}
